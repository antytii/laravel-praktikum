<x-auth-layout title="Register" section_title="Register" section_description="Register using your email">
    <form action="{{ route('auth.store') }}" method="POST" class="flex flex-col gap-4 mt-2">
        ...
        <div class="flex flex-col gap-2">
            ...
            <input type="text" id="name"name="name"  class="px-3 py-2 border border-zinc-300 bg-slate-50" placeholder="Your name" value="{{ old('name') }}">
            @error('name')
        ...
        <div class="flex flex-col gap-2">
            ...
            <input type="password" id="confirm_password" name="confirm_password" class="px-3 py-2 border border-zinc-300 bg-slate-50"
        ...
        <button type="submit" class="bg-blue-500 text-white px-3 py-2 text-center gap-2 cursor-pointer mt-4">
            <span>Register</span>
        </button>
        <p class="text-zinc-600 text-sm text-center">
            Already have an account? <a href="{{ route('auth.login') }}" class="text-blue-500 font-semibold underline">Login Here!</a>
        </p>
    </form>
</x-auth-layout>